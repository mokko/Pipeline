<xsl:stylesheet version="2.0"
    ...
    <xsl:template name="objectWorkTypeWrap">
        <lido:objectWorkTypeWrap>
            <xsl:apply-templates mode="workType"
                select="sachbegriff[not(@art = 'Einheimische Bezeichnung (lokal)')]">
                <xsl:sort select="@art" />
            </xsl:apply-templates>
            <xsl:apply-templates select="sachbegriffHierarchisch"/>
            <xsl:if test="not (sachbegriff[not(@art = 'Einheimische Bezeichnung (lokal)')]) 
                and not (sachbegriffHierarchisch)">
                <lido:objectWorkType>
    ...
    <xsl:template match="/museumPlusExport/sammlungsobjekt/sachbegriffHierarchisch">
        <lido:objectWorkType>
