...
def test_identNr(xml_fn):
    ...
    r = tree.xpath(
        "/m:museumPlusExport/m:sammlungsobjekt[not(m:identNr)]",
...
def test_kein_sachbegriff(xml_fn):
    ...
    tree = etree.parse(xml_fn)
    r = tree.xpath(
        "/m:museumPlusExport/m:sammlungsobjekt[not(m:sachbegriff)]/@objId",
        namespaces={"m": "http://www.mpx.org/mpx"},
    )
    for fehler in r:
...
def test_mume_pfad(mpx_fn):
    ...
    s = mpx.xpath(
        "/m:museumPlusExport/m:multimediaobjekt[m:pfadangabe]",
        namespaces={"m": "http://www.mpx.org/mpx"},
    )
    for mume in s:
...
def standardbild_veröffentlichen(mpx_fn):
    ...
    mpx = etree.parse(mpx_fn)
    s = mpx.xpath(
        "/m:museumPlusExport/m:multimediaobjekt[m:standardbild]",
        namespaces={"m": "http://www.mpx.org/mpx"},
    )
    e = []
    for mume in s:
        ...
        veröff = mume.xpath(
            "m:veröffentlichen", namespaces={"m": "http://www.mpx.org/mpx"}
        )
        if len(veröff) > 0:
...
def anzahl_definitiver_STO(mpx_fn):
    ...
    mpx = etree.parse(mpx_fn)
    r = mpx.xpath(
        "/m:museumPlusExport/m:sammlungsobjekt[m:standort]",
        namespaces={"m": "http://www.mpx.org/mpx"},
    )
    e = []
    for so in r:
        ...
        sto_s = so.xpath(
            "m:standort[@status = 'Definitiv' and @art = 'Ständiger Standort']",
            namespaces={"m": "http://www.mpx.org/mpx"},
        )
        sto_a = so.xpath(
            "m:standort[@status = 'Definitiv' and @art = 'Aktueller Standort']",
            namespaces={"m": "http://www.mpx.org/mpx"},
        )
...
def sto_inkongruenz(mpx_fn):
    ...
    mpx = etree.parse(mpx_fn)
    r = mpx.xpath(
        "/m:museumPlusExport/m:sammlungsobjekt",
        namespaces={"m": "http://www.mpx.org/mpx"},
    )
    e = []
    for so in r:
        ...
        # aktueller Standort
        nodes = so.xpath(
            "m:aktuellerStandort", namespaces={"m": "http://www.mpx.org/mpx"}
        )
        for n in nodes:
            aktSto = n.text
        ...
        # Ständiger Standort
        nodes = so.xpath(
            "m:ständigerStandort", namespaces={"m": "http://www.mpx.org/mpx"}
        )
        for n in nodes:
            stoge_st = n.text
